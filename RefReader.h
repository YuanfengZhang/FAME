...
unsigned int readBufferSlice(const char* const start, const unsigned int offset, std::string& chromSeq);
...
inline void extractIdLines(const char* const start, const unsigned int offset, std::list<struct idPos>& idQueue)
{
    ...
    for(const char* idLine = start; (idLine = (const char*) memchr(idLine, '>', (start + offset) - idLine)); )
    {

        const char* startSec = ((const char*) memchr(idLine, '\n', (start + offset) - idLine)) + 1;
